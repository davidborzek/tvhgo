...
func TestGetAllSucceeds(t *testing.T) {
	...
	assert.Equal(t, int64(20), recordings.Total)
	assert.Len(t, recordings.Entries, 1)

	assert.Equal(t, dvrGridEntry.Channel, recordings.Entries[0].ChannelID)
	assert.Equal(t, dvrGridEntry.Channelname, recordings.Entries[0].ChannelName)
	assert.Equal(t, int64(dvrGridEntry.Create), recordings.Entries[0].CreatedAt)
	assert.Equal(t, int64(dvrGridEntry.Duration), recordings.Entries[0].Duration)
	assert.Equal(t, dvrGridEntry.Enabled, recordings.Entries[0].Enabled)
	assert.Equal(t, dvrGridEntry.StopExtra, recordings.Entries[0].EndPadding)
	assert.Equal(t, int64(dvrGridEntry.Stop), recordings.Entries[0].EndsAt)
	assert.Equal(t, dvrGridEntry.Filename, recordings.Entries[0].Filename)
	assert.Equal(t, dvrGridEntry.UUID, recordings.Entries[0].ID)
	assert.Equal(t, dvrGridEntry.Title, recordings.Entries[0].LangTitle)
	assert.Equal(t, int64(dvrGridEntry.StopReal), recordings.Entries[0].OriginalEndsAt)
	assert.Equal(t, int64(dvrGridEntry.StartReal), recordings.Entries[0].OriginalStartsAt)
	assert.Equal(t, dvrGridEntry.StartExtra, recordings.Entries[0].StartPadding)
	assert.Equal(t, int64(dvrGridEntry.Start), recordings.Entries[0].StartsAt)
	assert.Equal(t, dvrGridEntry.DispTitle, recordings.Entries[0].Title)
	assert.Equal(t, dvrGridEntry.SchedStatus, recordings.Entries[0].Status)
}
